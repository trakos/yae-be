<?php
	class YaeSearch
	{
		static protected function flag($ip)
		{
		...
		static public function format_value( $key, &$value, $type ) //$type = player or server
		{
		...
		static public function show_form( $form_name, $form_elements, $title, $left = true )
		{
		...
		static public function show_results( $fieldclick_name, $details_name, $fields, $data )
		{
			if(empty($data))
			{
			...
			foreach( $data as $row )
			{
				...
				{
					$show_value = self::format_value( $key, $value, (isset($row['nick']) || $key == "played from" || $key == "played to ")?"player":"server");
					if($key == "id" )
					{
						echo "<td><a href='?show=$details_name&amp;id=$value' rel='nofollow'>&raquo;</a></td>";
					}
					else if( !empty($fieldclick_name) && isset($fields[$key]) )
					{
						echo "<td><a href=\"?show=$fieldclick_name&amp;$key=".urlencode($value)."\"  rel='nofollow'>$show_value</a></td>";
					}
					else
					{
		...
		static public function show_description($name, $data, $fields, $onclick )
		{
			if(!$data)
			{
			...
			{
				$show_value = self::format_value( $key, $value, isset($data['nick'])?"player":"server");
				echo "<tr><th>$key:</th>";
		...
		static public function create_where( $keys )
		{
			$where = "WHERE 1 AND";
		...
		static public function create_args( $keys )
		{
			$args = array();
			foreach($keys as $key => $sqlkey)
			{
				if(isset($_GET[$key]) && !empty($_GET[$key]))
				{
					...
					{
						$value+=0; // odcinamy zera na poczatku
					}
					else if($key == 'ip')
					{
		...
		static public function show_pages($first_page,$last_page,$current_page,$limit)
		{
			...
			for( $i=$first_page; $i<=$last_page; $i++ )
			{
				if($i != $current_page)
				{
					...
					echo "<a href='$link' rel='nofollow'>$i</a> ";
				}
				else
				{
					echo "<span>$i</span> ";
