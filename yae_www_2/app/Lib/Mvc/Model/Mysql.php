 ...
 */
class Lib_Mvc_Model_Mysql extends mysqli
{
	 ...
	 * @throws Lib_Mvc_Model_Mysql_Exception
	 */
	public function __construct( $hostname, $user, $password, $database )
	{
		...
		{
			throw new Lib_Mvc_Model_Mysql_Exception( $this->connect_errno, $this->connect_error, self::E_SQL_CONNECT, "__construct", array() );
		}
	}
	...
	public function simplequery( $query, $resultmode=MYSQLI_STORE_RESULT, $args=array() )
	{
		...
		if( $this->errno )
		{
			throw new Lib_Mvc_Model_Mysql_Exception( $this->errno, $this->error, self::E_SQL_QUERY, $query, $args );
		}
		return $result;
	}
	...
	/**
	 * Puts arguments in place of '?' signs.
	 * @param $query
	 * @param $args
	...
	protected function parseQuery( $query, $args, $chars=-1 )
	{
		...
		if( $char < $chars )
		{
			if(empty($args))
			{
				throw new Lib_Mvc_Model_Mysql_Exception( -1, "Not enough arguments passed to a queryf-type function.", self::E_SQL_NOT_ENOUGH_ARGS, $query, $args );
			}
			else
			{
				$q1 = "";
				...
				switch (true)
				{
					...
					case is_object($arg):
						...
						break;
					default:
						throw new Lib_Mvc_Model_Mysql_Exception( -1, "Wrong type of argument!", self::E_SQL_WRONG_ARG_TYPE, $query, $args );
						break;
				}
				$q2 = "";
				if($char < $chars-1)
		...
		{
			throw new Lib_Mvc_Model_Mysql_Exception( -1, "Too much arguments passed to a queryf-type function.", self::E_SQL_TOO_MUCH_ARGS, $query, $args );
		}
		else
		{
			return $query;
